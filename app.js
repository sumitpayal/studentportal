...
const stSubmitAssignment= mongoose.model("stSubmitAssignment",submit_assignSchema);
...
function isLoggedIn(req, res, next) { 
  ...
  res.redirect("/log"); 
   ...
   passport.use("User-local",new LocalStrategy({usernameField: 'enroll',
   passwordField: 'password'},
       (enroll, password, done)=> { 
           Stud.findOne({enroll_id:enroll}, function (err, user) {
               if (err) { return done(err); }
               if (!user) {
                   return done(null, false, { message: "Invalid enrollment_id or password" });
               }
   ...
   app.post("/login",
   passport.authenticate("User-local", { successRedirect: "/studenthome",
                                    failureRedirect: "/log",
                                    failureFlash: true })                  
 );
...
app.post("/signin",function(req, res){
  Stud.countDocuments({enroll_id:req.body.enroll}).then((count)=>{
    if(count>0){  
      Stud.findOne({enroll_id:req.body.enroll},{password:1},(err,passData)=>{ 
        if(passData.password==null){
          
          if (!(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/).test(String(req.body.email))) {
            req.flash('message','Email is not valid');    
            return res.redirect('/signup');
            
           }
           if (!(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z@]{8,}$/).test(String(req.body.pass))) {
            req.flash('message','Password is not valid');    
            return res.redirect('/signup');
           }
           if (!(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z@]{8,}$/).test(String(req.body.conpass))) {
                req.flash('message','Password is not valid');    
                return res.redirect('/signup');
            }
      ...
      Stud.updateOne({enroll_id:req.body.enroll},{$set:{password:hash}},(err)=>{
        if (!err){
    ...
    'Enrollment Number: '+req.body.enroll +'\n\n Password:  '+req.body.pass +' \n\nhttp://' + req.headers.host + '/' + '\n\n'
              ...
              transporter.sendMail(mailOptions, function(error, info){
                if (error) {
                  ...
                  req.flash('message',''+ req.body.name +' you are successfully signed up');
                  res.redirect('/log');
                }
              });
        }
    ...
    {
      req.flash('message','Sorry,Enrollment id '+req.body.enroll +' is not recognized institute id');
      res.redirect('/signup'); 
    }
  });
});
...
app.post("/teacherSignin", (req, res) =>{
      teacher.countDocuments({t_id:req.body.enroll}).then((count)=>{
        if(count>0){ 
          teacher.findOne({t_id:req.body.enroll},{password:1},(err,passData)=>{
            if(passData.password==null){
          ...
          if(req.body.conpass==req.body.pass){
           const salt = bcrypt.genSaltSync(10);
          const hash = bcrypt.hashSync(req.body.pass, salt);
          teacher.updateOne({t_id:req.body.enroll},{$set:{password:hash}},(err)=>{
            if (!err){
              req.flash('message',''+ req.body.name +' you are successfully signed up');
          ...
          req.flash('message','Sorry,Teacher id '+req.body.enroll +' is not a recognized institute id');
          res.redirect('/teachSignup');
        }
      });
});

passport.use("teacher-local",new LocalStrategy({usernameField: 'enroll',
passwordField: 'password'},
    (enroll, password, done)=> {
        teacher.findOne({t_id:enroll}, function (err, Tuser) {
            if (err) { return done(err); }
            if (!Tuser) {
                return done(null, false, { message: "Invalid Teacher Id or password" });
            }
...
app.post("/teacherlogin",
  passport.authenticate("teacher-local", { successRedirect: "/faculty",
  ...
  async.waterfall([
    ...
    function(token, user, done) { 
      ...
      var mailOptions = {
        ...
        'http://' + req.headers.host + '/newPassword' + '\n\n' +
      ...
      transporter.sendMail(mailOptions, function(error, info){
        if (error) {
          ...
          req.flash('message','Please go to your mail to continue with change password');
          res.redirect('/log');
        }
      });
    }
...
app.post("/newStPass",function(req,res){ 
  Stud.findOne({email:Email}, function (err, user) {
    ...
    if (!user) { 
        ...
        if (!(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z@]{8,}$/).test(String(req.body.password))) {
          req.flash('message','Password is not valid');
          res.redirect('/newPassword');
        }
        if (!(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z@]{8,}$/).test(String(req.body.conpassword))) {
          req.flash('message','Password is not valid');
          res.redirect('/newPassword');
        } 
        ...
        if(req.body.conpassword==req.body.password){
         ...
         Stud.updateOne({email: Email},{ $set:{ password: hash}},(err)=>{
  ...
  async.waterfall([
    ...
    function(token, user, done) {
      ...
      var mailOptions = {
        ...
        text: 'You are receiving this because you (or someone else) have requested the reset of the password for your account.\n\n' +
        'Please click on the following link, or paste this into your browser to complete the process:\n\n' +
        'http://' + req.headers.host + '/TeachNewPass' + '\n\n' +
        'If you did not request this, please ignore this email and your password will remain unchanged.\n'
      };
  ...
  teacher.findOne({email:Email}, function (err, user) {
    ...
    if (!user) { 
        ...
        if(req.body.conpassword==req.body.password){
          const salt = bcrypt.genSaltSync(10);
          const hash = bcrypt.hashSync(req.body.password, salt);
         teacher.updateOne({email: Email},{ $set:{ password: hash}},(err)=>{
    req.flash('message','Password has been changed successfully');
    res.redirect('/teachLog');
           });
...
app.get('/note',isLoggedIn ,(req,res)=>{  
  ...
  picModel.find({course:Student_course,sem:Student_sem},(err,data)=>{  
          ...
          if(data){    
              res.render('note',{im:imageData,loginName:Student_name,data:data})  
          }   
         else{  
...
//Assignment code
   app.get("/assupcoming" , isLoggedIn,(req,res)=>{
    stAssignment.find({course:Student_course,sem:Student_sem},{},(err,data)=>{
      res.render("assupcoming",{im:imageData,loginName:Student_name,message:req.flash('message'),data:data});
    });
  });

app.get('/StudentUpAssign/:id/:sub',isLoggedIn ,(req,res)=>{
  res.render('StudentUpAssign',{im:imageData,loginName:Student_name,data:req.params.id,data1:req.params.sub});
});
...
app.get("/submittedassignmentSt" , isLoggedIn,(req,res)=>{
  stSubmitAssignment.find({enroll_id:Student_enroll_id},{},(err,data)=>{
    if(!err)
    res.render("submittedassignmentSt",{loginName:Student_name,im:imageData,message:req.flash('message'),data:data} );
  })
});

app.get("/StudentReUpAssign/:id/:sub",isLoggedIn,(req,res)=>{ 
  res.render('StudentReUpAssign',{loginName:Student_name,im:imageData,data:req.params.id,data1:req.params.sub});
});

app.post("/AssignReupload/:id",upload.single('pic'),(req,res)=>{
  var Path='uploads/'+fileTime+req.file.originalname; 
  stSubmitAssignment.updateOne({_id:req.params.id},{$set:{st_message:req.body.msg,fileuploadPath:Path}},(err)=>{
    if(!err){
...
app.get("/attendance", isLoggedIn,(req,res)=>{
  ...
  stAttendance.aggregate([{$match:{enroll_id:Student_enroll_id}},{$group:{_id:{subject:"$subject"},count:{$sum:1}}}, {$sort:{"_id.subject":1}}],function(err, user){  
  array =user;
  array.forEach(function(current_value) {
  });
  });
  
   stAttendance.aggregate([{$match:{enroll_id:Student_enroll_id,attend:"Absent"}},{$group:{_id:{subject:"$subject"},count:{$sum:1}}}, {$sort:{"_id.subject":1}}],function(err, user){
     test =user;
  ...
  stAttendance.aggregate([{$match:{enroll_id:Student_enroll_id,attend:"Present"}},{$group:{_id:{subject:"$subject"},count:{$sum:1}}},{$sort:{"_id.subject":1}}],function(err, user){
    array1 =user;
...
res.render("attendance",{im:imageData,loginName:Student_name,data:array});
  });
});


app.get("/subjectattendance",isLogged ,(req,res)=>{
  stAttendance.find({enroll_id:Student_enroll_id,subject:req.body.subjectId},{},(err,pData)=>{ 
...
app.get("/attendSpecific", isLoggedIn,(req,res)=>{ 
  stAttendance.find({},{},(err,attendData)=>{
    if(!err){
      res.render("attendSpecific",{im:imageData,loginName:Student_name,data:attendData});
    }
  });
});

app.get('/attendanceof/:id', isLoggedIn,(req,res)=>{ 
  stAttendance.find({subject:req.params.id,enroll_id:Student_enroll_id},{},(err,attendData)=>{
    if(!err)
    res.render("attendSpecific",{im:imageData,loginName:Student_name,data:attendData,subName:req.params.id});
  });
}); 
//attendance code ends here
  ...
  app.get("/studenthome", isLoggedIn,function(req,res){
    ...
    {
      Stud.find({_id:req.session.passport.user.userId},{image:1,enroll_id:1,name:1,course:1,sem:1,_id:0},(err,sesData)=>{
      ...
      Stud.find({_id:req.session.passport.user.userId},{},(err,stData)=>{
        stAssignment.find({course:Student_course,sem:Student_sem},{assPath:1},(err,assignData)=>{
          tNotice.find({},{subject:1},(err,nData)=>{
            picModel.find({course:Student_course,sem:Student_sem},{subject:1},(err,comData)=>{ 
                if(!err)
                {
                  res.render("studenthome",{loginName:Student_name,message:req.flash('message'),im:imageData,cData:comData,noticeData:nData,data:stData,aData:assignData});
                }    
            }).limit(3).sort({date:-1});
          }).limit(3).sort({date:-1});
      }).limit(3).sort({date:-1});
...
app.post('/uploadImage',upload.single('fileInput'),(req,res)=>{
  ...
  Stud.updateOne({_id:req.session.passport.user.userId},{$set:{image:imagePath}},(err)=>{
  ...
  app.post("/mobilechange",(req,res)=>{
    Stud.updateOne({_id:req.session.passport.user.userId},{$set:{mob:req.body.newMobile}},(err)=>{
      req.flash('message','Mobile Number '+ req.body.newMobile+' has been updated successfully');
     res.redirect('/studenthome');
    });
  });
  
  app.post("/addresschange",(req,res)=>{
    Stud.updateOne({_id:req.session.passport.user.userId},{$set:{address:req.body.newAddress}},(err)=>{
      req.flash('message','New address '+ req.body.newAddress+' has been updated successfully');
      res.redirect('/studenthome');
    });
  });
  
  app.post("/emailchange",(req,res)=>{
    Stud.updateOne({_id:req.session.passport.user.userId},{$set:{email:req.body.newEmail}},(err)=>{
      req.flash('message','Email id '+ req.body.newEmail+' has been updated successfully');
      res.redirect('/studenthome');
   });
  });
...
app.get("/studentTest", isLoggedIn,(req,res)=>{
  ...
  stTest.distinct('subject',(err,subData)=>{
     if(!err)
      res.render("studentTest",{im:imageData,loginName:Student_name,testData:{},sData:subData,eData:examData});
  });
  });
  });
  ...
  app.post("/studentSearch",(req,res)=>{
    ...
    stTest.distinct('subject',(err,subData)=>{
        ...
        {
          stTest.find({exam_name:req.body.exam,enroll_id:Student_enroll_id},{},(err,testRecord)=>{
            if(!err)
              res.render("studentTest",{im:imageData,loginName:Student_name,testData:testRecord,sData:subData,eData:examData});
          }).sort({enroll_id:1,marks:-1});
        }
        else if(req.body.exam=="Select a exam")
        {
          stTest.find({subject:req.body.sub,enroll_id:Student_enroll_id},{},(err,testRecord)=>{
            if(!err)
              res.render("studentTest",{im:imageData,loginName:Student_name,testData:testRecord,sData:subData,eData:examData});
          }).sort({enroll_id:1,marks:-1});
        }
        else if(req.body.exam!="Select a exam" && req.body.sub!="Select a subject"){
         stTest.find({subject:req.body.sub,exam_name:req.body.exam,enroll_id:Student_enroll_id},{},(err,testRecord)=>{
          if(!err)
            res.render("studentTest",{im:imageData,loginName:Student_name,testData:testRecord,sData:subData,eData:examData});
         }).sort({enroll_id:1,marks:-1});
        }
...
app.get("/complaint", isLoggedIn,(req,res)=>{
  stComplain.find({enroll_id:Student_enroll_id},{},(err,comp)=>{ 
      if(!err)
      res.render("complaint",{im:imageData,loginName:Student_name,compData:comp,message:req.flash('message')}); 
 });
});
...
app.get("/studentNotice", isLoggedIn,(req,res)=>{
  tNotice.find({},{subject:1,message:1,date:1},function(err,noticeData){
    if(!err)
      res.render("studentNotice",{im:imageData,loginName:Student_name,cirData:noticeData});
  }).sort({"date":-1});
});
    ...
    var array=[];
      teacher.find({t_id:FacultyId},{subject:1,_id:0},(err,subData)=>{
        Stud.distinct('course',(err,courseData)=>{
          ...
          if(data){  
            res.render('teacherassign',{loginName:FacultyName,data:data,message : req.flash('message'),courseData:courseData,subData:teacher_subject_arrray.toString().split(',')})  
        }   
       else{  
           res.render('teacherassign',{loginName:FacultyName,data:{},message : req.flash('message'),courseData:courseData,subData:teacher_subject_arrray.toString().split(',')})  
         } 
        });
    });
  });
...
app.post('/teacherassign',isLogged,upload.single('pic'),(req,res)=>{  
  ...
  var ar;
  today = yyyy + '-' + mm + '-' + dd;
  stAssignment.find({},{A_id:1,_id:0},(err,data)=>{
    if(data.length==0){
      ar=0;
    }
    else{
    ...
    sta.save((err,data)=>{  
       if(!err){   
        req.flash('message','Assignment of '+ req.body.sub+' has been uploaded successfully');
...
app.get("/AssignmentSubmission",isLogged,(req,res)=>{
  stSubmitAssignment.find({},{},(err,data)=>{
    if(!err){
      res.render("AssignmentSubmission",{data:data,loginName:FacultyName});
    }
  }).sort({enroll_id:1});
});

app.get('/AssignSubmission/:id',isLogged,(req,res)=>{
  stSubmitAssignment.find({A_id:req.params.id},{},(err,data)=>{
    if(!err){
      res.render("AssignmentSubmission",{data:data,loginName:FacultyName});
    }
  }).sort({enroll_id:1});
...
app.get('/SubAssignDownload/:id',isLogged,(req,res)=>{
    ...
    picModel.find({t_id:FacultyId},{},(err,data)=>{  
      teacher.find({t_id:FacultyId},{subject:1,_id:0},(err,subData)=>{
        Stud.distinct('course',(err,courseData)=>{
          ...
          if(data){  
            res.render('facultynote',{loginName:FacultyName,data:data,courseData:courseData,subData:array.toString().split(','),message : req.flash('message')})  
          }   
          else{  
          res.render('facultynote',{loginName:FacultyName,data:{},courseData:courseData,subData:array.toString().split(','),message : req.flash('message')})  
          } 
        });
      });  
    });        
  ...
  app.post('/facultynote',isLogged ,upload.single('pic'),(req,res)=>{  
    ...
    picss.save((err,data)=>{  
         if(!err){   
           req.flash('message',''+ req.file.originalname +' has been uploaded successfully');
...
app.get('/faculty' , isLogged ,function(req,res){
  if(req.session.passport.user.userGroup=="Teacher")
    {
      teacher.find({_id:req.session.passport.user.userId},{t_name:1,t_id:1,subject:1,_id:0},(err,sesData)=>{
        for(var kkey in sesData){
          if(sesData.hasOwnProperty(kkey)){
      ...
      teacher.find({_id:req.session.passport.user.userId},{},(err,data)=>{
        res.render("faculty",{loginName:FacultyName,data:data, message:req.flash('message')});
      });
    }
    else
    {
  ...
  app.post("/teachmobilechange",(req,res)=>{
    teacher.updateOne({_id:req.session.passport.user.userId},{$set:{mob:req.body.newteachMobile}},(err)=>{
      req.flash('message','Mobile number '+ req.body.newteachMobile+' has been updated successfully'); 
      res.redirect('/faculty');
    });
  });
  
  app.post("/teachaddresschange",(req,res)=>{
    teacher.updateOne({_id:req.session.passport.user.userId},{$set:{address:req.body.newteachAddress}},(err)=>{
      req.flash('message','New Address '+ req.body.newteachAddress+' has been updated successfully');  
      res.redirect('/faculty');
    });
  });
  
  app.post("/teachFatherUpdate",(req,res)=>{
    teacher.updateOne({_id:req.session.passport.user.userId},{$set:{fname:req.body.newteachFather}},(err)=>{
      req.flash('message','Father Name '+ req.body.newteachFather+' has been updated successfully');  
      res.redirect('/faculty');
   });
  });
  
  app.post("/teachemailchange",(req,res)=>{
    teacher.updateOne({_id:req.session.passport.user.userId},{$set:{email:req.body.newteachEmail}},(err)=>{
req.flash('message','Email Id  '+ req.body.newteachEmail+' has been updated successfully');
        res.redirect('/faculty');
   });
  });
...
app.get("/teachTest",isLogged ,(req,res)=>{
  Stud.distinct('course',(err,courseData)=>{
    res.render("teachTest",{loginName:FacultyName,message : req.flash('message'),courseData:courseData,data:{}});
  });
});

app.post("/enter_marks" ,isLogged,(req,res)=>{
  Stud.distinct('course',(err,courseData)=>{
    Stud.find({course:req.body.cour,sem:req.body.semester},function(err,markData){
      var array=[];
        teacher.find({t_id:FacultyId},{subject:1,_id:0},(err,subData)=>{
        for(var key in subData){
          if(subData.hasOwnProperty(key)){
        ...
        if(!err)
           res.render("teachTest",{loginName:FacultyName,message:{},courseData:courseData,data:markData,subData:array.toString().split(',')});
    });
    }).sort({enroll_id:1});
  });
});
...
var j=4;
var resTemp;
Stud.find({course:req.params.cour,sem:req.params.seme},{},(err,data)=>{
var comp=array[3];
for(var i in data){
...
req.flash('message','Marks of '+ array[0]+' subject for '+ array[1]+' exam has been uploaded successfully');
res.redirect('/teachTest');
});

app.get("/teach_checkmarks" ,isLogged,(req,res)=>{  
      stTest.distinct('exam_name',(err,examData)=>{ 
          res.render("teach_checkmarks",{loginName:FacultyName,testData:{},examData:examData,subData:teacher_subject_arrray.toString().split(',')});
      });  
});
...
{  
  stTest.find({exam_name:req.body.exam,subject:req.body.sub},{},(err,tsData)=>{ 
   stTest.distinct('exam_name',(err,examData)=>{ 
    if(!err)
      res.render("teach_checkmarks",{loginName:FacultyName,examData:examData,testData:tsData,subData:teacher_subject_arrray.toString().split(',')});
  });
    }).sort({enroll_id:1});
}
///
...
{  
stTest.find({subject:req.body.sub,exam_name:req.body.exam,enroll_id:req.body.enroll},{},(err,tsData)=>{  
  stTest.distinct('exam_name',(err,examData)=>{ 
    if(!err)
      res.render("teach_checkmarks",{loginName:FacultyName,examData:examData,testData:tsData,subData:teacher_subject_arrray.toString().split(',')});
  });}).sort({enroll_id:1});
...
{ 
stTest.find({enroll_id:req.body.enroll,subject:req.body.sub},{},(err,tsData)=>{
  stTest.distinct('exam_name',(err,examData)=>{ 
    if(!err)
      res.render("teach_checkmarks",{loginName:FacultyName,examData:examData,testData:tsData,subData:teacher_subject_arrray.toString().split(',')});
  });}).sort({enroll_id:1});
...
app.get("/teachcomplaint",isLogged ,(req,res) =>{
  stComplain.find({},{},function(err,complainData){
    if(!err)
    res.render("teachcomplaint",{loginName:FacultyName,comData:complainData});
 });
});
  ...
  tNotice.find({},{subject:1,message:1,date:1},function(err,noticeData){
    if(err)
     console.log(err);
      ...
      res.render("post_notice",{loginName:FacultyName,message : req.flash('message'),cirData:noticeData});
  });
});
...
app.get("/markattendance",isLogged ,(req,res)=>{
  Stud.distinct('course',(err,courseData) =>{
    Stud.find((err)=>{
          res.render("markattendance",{loginName:FacultyName,data1:courseData,message : req.flash('message'),data:{},subData:{}});
 });
});
});
...
app.post("/get-data",isLogged ,function(req,res){
  Stud.distinct('course',(err,courseData)=>{
    Stud.find({course:req.body.cour,sem:req.body.semester},(err,studData)=>{
      var array=[];
        teacher.find({t_id:FacultyId},{subject:1,_id:0},(err,subData)=>{
        for(var key in subData){
          if(subData.hasOwnProperty(key)){
        ...
        if(!err)
        res.render("markattendance",{ loginName:FacultyName,message:{},data:studData,data1:courseData,subData:array.toString().split(',')});
      });
    }).sort({enroll_id:1});
  });
});
...
today = yyyy + '-' + mm + '-' + dd;
  ...
  var arl=ar.length;
  Stud.find({course:req.params.cou,sem:req.params.seme},{},(err,data)=>{
    for(var i in data){
      if(data.hasOwnProperty(i)){
    ...
    req.flash('message','Attendance of '+ar[arl-1] +' is marked successfully');
...
app.get("/checkstudentattend",isLogged ,(req,res)=>{ 
  ...
  for(var key in subData){
    ...
    res.render("checkstudentattend",{ attData:{},loginName:FacultyName,subData:teacher_subject_arrray.toString().split(',')});
  });
});

app.post("/checkAttendanceStudent" ,isLogged,(req,res) =>{
  var array=[];
  stAttendance.find({enroll_id:req.body.enroll,subject:req.body.sub},{},(err,attData)=>{
    teacher.find({t_id:1},{subject:1,_id:0},(err,subData)=>{
      for(var key in subData){
        if(subData.hasOwnProperty(key)){
         array.push(subData[key].subject);
        }}
     res.render("checkstudentattend",{attData:attData,loginName:FacultyName,subData:array.toString().split(',')});
    });
  });
});
...
app.get("/checksubjectattend" ,isLogged,(req,res)=>{
  ...
  for(var key in subData){
    ...
    res.render("checksubjectattend",{loginName:FacultyName,data:{},subData:array.toString().split(',')});
  });
});

app.post("/checksubjectattendance",isLogged,(req,res)=>{
  var array=[];
  stAttendance.find({subject:req.body.sub,date:req.body.attDate},function(err,attendData1){
    teacher.find({t_id:1},{subject:1,_id:0},(err,subData)=>{
      for(var key in subData){
        if(subData.hasOwnProperty(key)){
         array.push(subData[key].subject);
   ...
   res.render("checksubjectattend",{loginName:FacultyName,data:attendData1,subData:array.toString().split(',')});
  }
    });
  });
...
//enroll_id value will be replaced by session id
 ...
 app.post('/Admin_login',(req,res)=>{
    adminDb.countDocuments({admin_id:req.body.aid,password:req.body.apass}).then((count)=>{
      if(count>0)
      {
        res.redirect('/admin');
      }
...
app.get('/AdminForget',(req,res)=>{
 ...
 app.post('/AdminForgetPass',(req,res)=>{
  adminDb.countDocuments({aadmin_id:req.body.aid}).then((count)=>{
    ...
    {
      adminDb.updateOne({admin_id:req.body.aid},{$set:{
...
app.post('/editstudent2/:id',(req,res)=>{
  Stud.updateOne({_id:req.params.id}, 
    {$set: {
    ...
    sem:req.body.seme,
    bsy:req.body.bsy1,
    bey:req.body.bey1}},(err)=>{
      if(!err){
        req.flash('message','Record of Enrollment Id '+ req.body.enroll1+' has been updated successfully');
...
app.post("/deletestudent",(req,res) =>{
  Stud.countDocuments({enroll_id:req.body.Denroll}).then((count)=>{
    ...
    Stud.deleteOne({enroll_id:req.body.Denroll},(err)=>{
      req.flash('message','Record of student id '+ req.body.Denroll +' has been successfully deleted');
      res.redirect('/admin');
    });
  }
  else{
    req.flash('message','Record of student id '+ req.body.Denroll +' is not available');
    res.redirect('/admin');
  }
  });
...
app.post('/editteacher2/:id',(req,res)=>{
  teacher.update({_id:req.params.id},{$set:{
    ...
    subject: req.body.sub
      ...
      if(!err){
        req.flash('message','Record of Teacher Id '+ req.body.tid+' has been updated successfully');
  ...
  teacher.countDocuments({t_id:req.body.Dtid}).then((count)=>{
    if(count>0)
    {
      teacher.deleteOne({t_id:req.body.Dtid},(err)=>{
        req.flash('message','Record of teacher id '+ req.body.Dtid+' has been successfully deleted');
        res.redirect('/admin');
      });
    }
    else{
      req.flash('message','Record of teacher id '+ req.body.Dtid+' is not available');
      res.redirect('/admin');
    }
  });
});
...
app.get("/addteacher",(req,res) =>{
  ...
  app.post("/addCourse",(req,res)=>{
    ...
    newCourse.save(function(err){
     req.flash('message','Course '+ req.body.cour +' has been added successfully');
      res.redirect('/admin');
    });
  });
